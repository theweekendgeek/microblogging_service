...
func TestParsesTimelines(t *testing.T) {
	...
	if assert.NotNil(t, timelineResponse.Posts) && assert.NotNil(t, timelineResponse.MetaData) {
		fmt.Println(timelineResponse.Posts[0].PostContent)
		assert.Equal(t, "1582051114941566976", timelineResponse.Posts[0].PostID)
		assert.Equal(t, "Your delivery is being prepared. 📦\n\nNorthrop Grumman's next cargo mission to the @Space_Station will deliver new experiments studying the dynamics of mudflows, growing crops in space, and fertility treatments—all for the benefit of humanity: https://t.co/bGCuPQv6a1 https://t.co/JLZWuwCJJx", timelineResponse.Posts[0].PostContent)
		assert.Equal(t, "1581056289282482176", timelineResponse.Posts[6].PostID)
		assert.Equal(t, "RT @nasahqphoto: Welcome home! Crew-4 NASA astronauts @astro_watkins, @astro_farmerbob, @astro_kjell, and @ESA’s @AstroSamantha are seen in…", timelineResponse.Posts[6].PostContent)
